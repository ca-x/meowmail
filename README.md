...
`0.7.0` 加入按用户启用的 OpenAI / Claude / Gemini 集成、邮件翻译与润色、自动标注规则及网络订阅、CalDAV 日历，并支持联系人拼音与中英文首字母检索；`0.6.1` 加入邮件过滤器、外出回复生效邮箱范围、附件发送、多选删除和 Gmail 代理连接诊断优化。`0.1.x` 从未上线，因此仍不包含从旧单用户原型迁移的逻辑。
...
- 联系人管理、拼音及中英文首字母检索、写信收件人自动完成、草稿箱、富文本写信和定时发送
- 可按用户启用 OpenAI Responses / Chat、Claude Messages 与 Gemini Generate Content；支持自定义兼容端点、HTTP / SOCKS5 代理、邮件翻译和写信润色，关闭后不显示 AI 功能
- AI 自动标注支持自定义标签、按邮箱生效的规则、手动或自动应用、JSON 导出和受限 HTTPS 网络订阅
- CalDAV 账户发现与同步、日历显示设置、月视图和按日事件列表
...
每位用户可以在“设置 → 安全 → MCP 访问”生成自己的 bearer token。MCP 地址固定为当前实例的 `/mcp`，例如：
